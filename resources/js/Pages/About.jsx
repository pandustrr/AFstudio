...
import { BuildingOfficeIcon, SparklesIcon, EnvelopeIcon, PhoneIcon, XMarkIcon, CameraIcon, ChevronLeftIcon, ChevronRightIcon, BoltIcon, HeartIcon, UserGroupIcon, ShieldCheckIcon, ChartBarIcon } from '@heroicons/react/24/outline';
...
export default function About({ about, moodboards = [] }) {
    ...
    const displayDNA = about?.dna?.length > 0 ? about.dna : defaultDNA;
    ...
    const displayStats = about?.stats?.length > 0 ? about.stats : defaultStats;
    ...
    return (
        <GuestLayout>
            ...
            <div className="min-h-screen bg-brand-white dark:bg-brand-black transition-colors duration-500 relative overflow-hidden">
                ...
                {/* Hero Section */}
                <section className="relative h-[60vh] md:h-[75vh] flex items-center justify-center overflow-hidden bg-brand-black">
                        ...
                        <div className="lg:col-span-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-5 pt-0 lg:pt-8">
                            {displayStats.map((stat, i) => (
                                <div key={i} className="relative group bg-white dark:bg-white/5 p-6 rounded-3xl border border-black/5 dark:border-white/10 overflow-hidden transition-all duration-700 hover:shadow-xl hover:border-brand-gold/40">
                                    <div className={`absolute top-0 right-0 w-24 h-24 bg-${i % 2 === 0 ? 'brand-gold' : 'brand-red'}/5 blur-2xl -mr-12 -mt-12 group-hover:opacity-100 opacity-0 transition-opacity`}></div>
                                    <div className="relative z-10 flex items-center justify-between">
                                        ...
                                        <ChartBarIcon className={`w-7 h-7 text-${i % 2 === 0 ? 'brand-gold' : 'brand-red'} opacity-20 group-hover:opacity-100 transition-all duration-700`} />
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        {displayDNA.map((pillar, i) => {
                            ...
                            return (
                                <div key={i} className="group p-8 bg-white dark:bg-white/5 rounded-3xl border border-black/5 dark:border-white/10 hover:border-brand-gold/30 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 relative overflow-hidden">
                                    ...
                                    <Icon className={`w-10 h-10 ${i % 2 === 0 ? 'text-brand-gold' : 'text-brand-red'} mb-6 transition-transform duration-500 group-hover:scale-110 group-hover:rotate-6`} />
                        ...
                        <div className="bg-brand-black dark:bg-white/5 text-white p-6 md:p-10 rounded-3xl relative overflow-hidden group shadow-2xl border border-white/5 transition-all duration-500">
                            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10 items-center">
                                <div className="space-y-4">
                                    ...
                                    <h3 className="text-2xl md:text-4xl font-black uppercase tracking-tighter italic leading-[0.85] text-white">
                                        {about?.cta_title ? (
                                            <>
                                                {about.cta_title.split(' ').slice(0, -1).join(' ')} <br /><span className="text-brand-gold tracking-[-0.05em]">{about.cta_title.split(' ').pop()}</span>
                                            </>
                                        ) : (
                                            <>READY TO <br /><span className="text-brand-gold tracking-[-0.05em]">IMMORTALIZE?</span></>
                                        )}
                                    </h3>
                                    <p className="text-white/40 text-[9px] font-medium max-w-sm uppercase tracking-widest leading-relaxed">
                                        {about?.cta_subtitle || 'Mari ciptakan masa depan yang abadi.'}
                                    </p>
                                </div>
