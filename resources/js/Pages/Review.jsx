                                ...
                                >
                                    ...
                                    <div className="relative mb-8">
                                        ...
                                        {review.additional_fields && Object.keys(review.additional_fields).length > 0 && (
                                            <div className="mt-6 flex flex-wrap gap-3 px-3 relative z-10">
                                                {Object.entries(review.additional_fields).map(([key, value]) => (
                                                    value && (
                                                        <div key={key} className="inline-flex flex-col items-start gap-1 p-3 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5 min-w-0 flex-1">
                                                            <span className="text-[8px] font-black uppercase tracking-widest text-brand-black/30 dark:text-brand-white/30 truncate w-full">
                                                                {key}
                                                            </span>
                                                            <div className="flex items-center gap-2 max-w-full">
                                                                ...
                                                                <span className="text-[10px] font-bold text-brand-black dark:text-brand-white truncate">
                                                                    {key.toLowerCase().includes('instagram') ? `@${value.replace('@', '')}` : value}
                                                                </span>
                                                            </div>
                                                        </div>
                                                    )
