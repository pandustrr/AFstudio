...
export default function SelectorPhoto() {
    ...
    const getTemplateValue = (key) => reviewTemplateArray.find(item => item.key === key)?.value || '';

    const customFields = reviewTemplateArray.filter(item => ['custom_field', 'client_instagram', 'client_tiktok'].includes(item.key));
    ...
    const fetchPhotosFromDrive = async (folderType, isRefresh = false) => {
        ...
        try {
            ...
            if (!response.ok) {
                const errorMsg = data.message || data.error || 'Gagal mengambil foto';
    ...
    const templateTypes = [
        ...
        { id: 'admin_instagram', name: 'Link IG Admin (Footer)', icon: '🔗' },
        { id: 'admin_tiktok', name: 'Link TikTok Admin (Footer)', icon: '🔗' },
        { id: 'admin_facebook', name: 'Link FB Admin (Footer)', icon: '🔗' },
    ];
    ...
    const handleSendReview = async () => {
        ...
        try {
            ...
            if (Object.keys(extraAnswers).length > 0) {
                formData.append('additional_fields', JSON.stringify(extraAnswers));
            }
            if (reviewPhoto) {
        ...
        <GuestLayout>
            ...
            <div className="relative z-10 pt-24 md:pt-32 pb-20 px-6 min-h-screen transition-colors">
                <div className={`relative z-10 mx-auto transition-all duration-500 ${step >= 3 ? 'max-w-2xl' : 'max-w-md'}`}>
                    ...
                    {(() => {
                        const reviewTitle = (driveType === 'Result' ? 'Kirim Ulasan' : (selectedPhotos.length > 0 ? 'Finalisasi Request' : 'Kirim Ulasan'));
                        const reviewSubtitle = "Berikan ulasan untuk sesi foto Anda.";
                        const reviewStarLabel = "Berikan Bintang";
                        const reviewPhotoLabel = "Upload Foto Kenangan (Opsional)";
                        const reviewPhotoButtonText = "Ambil Foto / Pilih File";
                        const reviewPhotoInstruction = "Maksimal 5MB";
                        const reviewTextareaLabel = (driveType === 'Result' ? 'Ulasan Anda' : (selectedPhotos.length > 0 ? 'Pesan Tambahan (Opsional)' : 'Ulasan Anda'));
                        const reviewTextareaPlaceholder = (selectedPhotos.length > 0 ? "Catatan untuk editor..." : "Tulis pengalaman Anda...");
                                    ...
                                    <div className="space-y-8 animate-fade-in">
                                        ...
                                        {!(driveType === 'Mentahan' && selectedPhotos.length > 0) && (
                                            <div className="space-y-6">
                                                ...
                                                <div className="flex flex-col items-center space-y-3">
                                                    <label className="text-[10px] uppercase font-black tracking-widest text-brand-black/40 dark:text-brand-white/40">{reviewStarLabel}</label>
                                                ...
                                                <div className="space-y-3">
                                                    <label className="block text-[10px] uppercase font-black tracking-widest text-brand-black/60 dark:text-brand-white/60">{reviewPhotoLabel}</label>
                                                        ...
                                                        <div className={`w-full bg-black/5 dark:bg-white/5 border-2 border-dashed ${reviewPhoto ? 'border-brand-gold bg-brand-gold/5' : 'border-black/10 dark:border-white/10 group-hover:border-brand-gold/40'} rounded-2xl p-4 transition-all overflow-hidden`}>
                                                            {reviewPhoto ? (
                                                                ...
                                                                <div className="py-8 text-center">
                                                                    ...
                                                                    <p className="text-[10px] font-black uppercase tracking-widest text-brand-black/40 dark:text-brand-white/40">{reviewPhotoButtonText}</p>
                                                                    <p className="text-[8px] font-bold text-brand-black/20 dark:text-brand-white/20 uppercase tracking-widest mt-1">{reviewPhotoInstruction}</p>
                                                                </div>
                                                            )}
                                                        </div>
                                                    </div>
                                                ...
                                                {/* Default Review Textarea - Only show if no custom form fields added */}
                                                {customFields.filter(f => f.key === 'custom_field').length === 0 && (
                                                    <div className="space-y-4">
                                                ...
                                                {customFields.map((field, i) => (
                                                    <div key={i} className="space-y-4 animate-fade-in" style={{ animationDelay: `${(i + 1) * 100}ms` }}>
                                                        <label className="block text-[10px] uppercase font-black tracking-widest text-brand-black/60 dark:text-brand-white/60 flex items-center gap-2">
                                                            {field.key === 'client_instagram' && <span className="text-brand-gold text-xs">📸</span>}
                                                            {field.key === 'client_tiktok' && <span className="text-brand-gold text-xs">🎵</span>}
                                                            {field.value}
                                                        </label>
                                                        <div className="relative group">
                                                            {field.key === 'client_instagram' && (
                                                                <span className="absolute left-6 top-1/2 -translate-y-1/2 text-brand-black/20 dark:text-white/20 font-bold text-sm">@</span>
                                                            )}
                                                            <input
                                                                type="text"
                                                                value={extraAnswers[field.value] || ''}
                                                                onChange={(e) => setExtraAnswers(prev => ({ ...prev, [field.value]: e.target.value }))}
                                                                placeholder={field.key === 'client_instagram' ? 'username_kamu' : '...'}
                                                                className={`w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl ${field.key === 'client_instagram' ? 'pl-11 pr-6' : 'px-6'} py-4 text-brand-black dark:text-brand-white focus:outline-none focus:border-brand-gold transition-all text-sm font-medium`}
                                                            />
                                                        </div>
                                                    </div>
                                                ))}
                                        ...
                                        <div className="flex gap-4">
                                            ...
                                            <button
                                                ...
                                                className="flex-2 bg-brand-gold hover:brightness-90 text-brand-black font-black py-4 rounded-xl uppercase tracking-widest text-xs shadow-xl disabled:opacity-50 transition-all"
                                            >
                                                {loading ? 'Mengirim...' : driveType === 'Mentahan' && selectedPhotos.length > 0 ? 'Kirim Request' : 'Kirim Ulasan'}
                                            </button>
                                        </div>
                                    </div>
                                )}
                    ...
                    <div className="mt-12 flex flex-wrap justify-center sm:justify-between items-center gap-6 px-4">
